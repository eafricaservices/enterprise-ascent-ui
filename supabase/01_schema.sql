...
CREATE OR REPLACE TRIGGER contact_submissions_updated_at
...
CREATE INDEX IF NOT EXISTS idx_talent_applications_email       ON public.talent_applications (email);
CREATE INDEX IF NOT EXISTS idx_talent_applications_country     ON public.talent_applications (country);
CREATE INDEX IF NOT EXISTS idx_talent_applications_created_at  ON public.talent_applications (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_talent_applications_skills      ON public.talent_applications USING GIN (skills);
CREATE INDEX IF NOT EXISTS idx_talent_applications_languages   ON public.talent_applications USING GIN (languages);

-- Ensure the same email cannot submit duplicate applications in 'submitted' state
CREATE UNIQUE INDEX IF NOT EXISTS idx_talent_applications_email_submitted
  ON public.talent_applications (email)
  WHERE status = 'submitted';

CREATE OR REPLACE TRIGGER talent_applications_updated_at
...
CREATE TABLE IF NOT EXISTS public.testimonials (
  ...
  UNIQUE (author_name, row_group)  -- prevents duplicate seeding and enforces one entry per person per row
);
...
DO $$ BEGIN
  ALTER TABLE public.testimonials
    ADD CONSTRAINT testimonials_author_row_unique UNIQUE (author_name, row_group);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_testimonials_active_row ON public.testimonials (is_active, row_group, sort_order);
...
CREATE TABLE IF NOT EXISTS public.faqs (
  id         UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  question   TEXT        NOT NULL UNIQUE CHECK (char_length(question) BETWEEN 5 AND 500),
...
DO $$ BEGIN
  ALTER TABLE public.faqs
    ADD CONSTRAINT faqs_question_unique UNIQUE (question);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.pricing_plans
    ADD CONSTRAINT pricing_plans_tier_unique UNIQUE (tier);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- Only one plan may be featured at a time
CREATE UNIQUE INDEX IF NOT EXISTS idx_pricing_plans_one_featured
  ON public.pricing_plans (is_featured)
  WHERE is_featured = TRUE;

CREATE INDEX IF NOT EXISTS idx_pricing_plans_active_order ON public.pricing_plans (is_active, sort_order);

CREATE OR REPLACE TRIGGER pricing_plans_updated_at
...
CREATE TABLE IF NOT EXISTS public.pricing_plan_features (
  ...
  UNIQUE (plan_id, feature_text)  -- prevents duplicate feature bullets within the same plan
);
...
DO $$ BEGIN
  ALTER TABLE public.pricing_plan_features
    ADD CONSTRAINT pricing_plan_features_plan_feature_unique UNIQUE (plan_id, feature_text);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_pricing_plan_features_plan ON public.pricing_plan_features (plan_id, sort_order);
...
CREATE TABLE IF NOT EXISTS public.client_logos (
  id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  company_name TEXT        NOT NULL UNIQUE CHECK (char_length(company_name) BETWEEN 1 AND 200),
...
DO $$ BEGIN
  ALTER TABLE public.client_logos
    ADD CONSTRAINT client_logos_company_name_unique UNIQUE (company_name);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_client_logos_active_order ON public.client_logos (is_active, sort_order);

CREATE OR REPLACE TRIGGER client_logos_updated_at
...
DO $$ BEGIN
  ALTER TABLE public.site_stats
    ADD CONSTRAINT site_stats_label_unique UNIQUE (label);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_site_stats_active_order ON public.site_stats (is_active, sort_order);
...
DO $$ BEGIN
  ALTER TABLE public.impact_items
    ADD CONSTRAINT impact_items_title_unique UNIQUE (title);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_impact_items_active_order ON public.impact_items (is_active, sort_order);

CREATE OR REPLACE TRIGGER impact_items_updated_at
...
ON CONFLICT (plan_id, feature_text) DO NOTHING;
...
ON CONFLICT (author_name, row_group) DO NOTHING;
